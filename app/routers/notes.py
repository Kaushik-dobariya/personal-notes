...
async def create_note(
    ...
    note = await note_service.create_user_note(current_user.id, data)
...
async def update_note(
    ...
    """
    Update note title and content via full page form or API.
    Strictly verifies ownership: returns 404 if note belongs to another user.
    """
    note_service = NoteService(db)
    ...
    try:
        updated_note = await note_service.update_user_note(note_id, current_user.id, data)
    except ValueError:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Note not found")
